            ...
            <div x-ref="slider" @scroll="updateArrows()" class="flex gap-4 md:gap-6 overflow-x-hidden scroll-smooth relative" data-aos="fade-up">
                @forelse ($blogs as $index => $blog)
                    <div class="relative group overflow-hidden w-[280px] md:w-[530px] h-[195px] md:h-[369px] flex-shrink-0">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-4 md:p-8 transition-opacity duration-300" :class="isCardPartial({{ $index }}) ? 'opacity-50' : 'opacity-100'">
                            <h2 class="text-white text-[16px] md:text-[20px] mb-[10px] md:mb-[15px] font-andada-semibold">{{ $blog->title }}</h2>
                            <p class="text-white text-[14px] md:text-[18px] leading-relaxed font-joan line-clamp-3 opacity-80">
                                {{ $blog->excerpt }}
                            </p>
                        </div>
                    </div>
